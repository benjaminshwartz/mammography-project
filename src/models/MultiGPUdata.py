...
def get_train_test_dataloader(split: tuple, sequential: bool, batch: int, path:str = None) -> Tuple[DataLoader, DataLoader]:
    ...
    training_generator = DataLoader(train_set, batch_size = batch, shuffle = False,sampler=DistributedSampler(train_set))
    test_generator = DataLoader(test_set, batch_size = batch, shuffle = False, sampler=DistributedSampler(test_set))
